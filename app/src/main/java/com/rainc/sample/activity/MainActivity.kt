        ...
        rows = listOf(
            ...
            Row(
                ...
                cells = listOf(
                    ...
                    ButtonCell("Cell 17", Coordinate(4, 2), hasError = true),
                    ...
                    onCellAction = { action ->
                        when(action){
                            is CellAction.ButtonPressed -> {
                                when(action.cell){
                                    is ButtonCell -> {
                                        tableState.value = tableState.value.copy(
                                            rows = tableState.value.rows.map { row ->
                                                row.copy(
                                                    cells = row.cells.map { cell ->
                                                        when(cell){
                                                            is ButtonCell -> {
                                                                if(cell.uuid == action.cell.uuid){
                                                                    cell.copy(hasError = cell.hasError.not())
                                                                }
                                                                else cell
                                                            }
                                                            else -> cell
